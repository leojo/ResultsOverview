...
class config(object):
	...
	def get_validation_batch(self):
		...
		samples = []
		originals = []
		for i in self.validation_indices[0]:
			for j in self.validation_indices[1]:
				if len(self.data_sources) == 1:
					if i == j:
						continue
					wave1 = self.data[0][i]
		...
		samples = np.asarray(samples[:self.batch_size])[p]
		originals = np.asarray(originals[:self.batch_size])[p]
		return samples, originals
