...
class config(object):
	...
	def load_and_prepare_audio(self, source, i):
		...
		if i == 0:
			cut_data = waveUtils.extractRandomSegments(waves, num_per_source=30, sample_rate=original_sample_rate, duration=duration)
		else:
			cut_data = waveUtils.extractRandomSegments(waves, num_per_source=5, sample_rate=original_sample_rate, duration=duration, max_intensity=0.5)
		del waves
